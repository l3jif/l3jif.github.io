...
class Discord:
    ...
    def sendMessage(self, message_discord:str, discord_name:str, discord_icon:str, image_path:str=None) -> dict:
        """
        Discord の Webhook でメッセージを送信する

        Args:
            message_discord (str): 送信するメッセージの本文
        ...
        if response.status_code != 200 and response.status_code != 204:
            message_discord = response.json()['message'] + f' (Code: {response.json()["code"]})'
        else:
            message_discord = 'Success'

        # ステータスコードとエラーメッセージを返す
        return {
            'status': response.status_code,
            'message': message_discord,
        }
